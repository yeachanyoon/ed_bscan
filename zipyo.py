...
if __name__ == "__main__":
    ...
    # 2. 모델 실행
    model = EllipticalDBSCAN(L=1.5, min_pts=6, angle_threshold_degrees=27, min_branch_size=10)
    labels = model.fit(X)

    # 3. 클러스터링 성능 평가 (추가된 부분)
    # 노이즈(-1)를 제외하고 평가하거나, 포함해서 평가할 수 있음 (여기서는 포함)
    # 단, 군집이 2개 이상 형성되어야 지표 계산 가능
    ...
    if n_clusters > 1:
        ...
        # Davies-Bouldin Index: 낮을수록 좋음 (0에 가까울수록 좋음)
    ...
    plt.title('Slope-Sensitive Elliptical DBSCAN with Metrics')
